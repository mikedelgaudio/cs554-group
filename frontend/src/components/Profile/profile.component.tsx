    ...
    if (isCurrentUser) {
        ...
        return (
            <div className="flex flex-col items-center justify-center w-full h-full">
                ...
                {/* Edit profileImage */}
                <form className="form">
                    <input type="text" name="profileImage" id="profileImage" className='input'/>
                    <button type="submit">Submit</button>
                </form>
                <br/>
                <h3>Phone Number: {useCurrentUser().contactInfo.phoneNumer}</h3>
                <h3>Email: {useCurrentUser().contactInfo.email}</h3>
                <h3>Personal Website: {useCurrentUser().contactInfo.personalWebsite}</h3>
                <h3>Current Role: {useCurrentUser().contactInfo.currentRole}</h3>
                {/* Edit contactInfo */}
                <form className="form">
                    <input type="text" name="phoneNumer" id="phoneNumer" className='input'/>
                    <input type="text" name="email" id="email" className='input'/>
                    <input type="text" name="personalWebsite" id="personalWebsite" className='input'/>
                    <input type="text" name="currentRole" id="currentRole" className='input'/>
                    <button type="submit">Submit</button>
                </form>
                <br/>
                <h3>Social Media: {useCurrentUser().socialMedia.map((socialMedia) => socialMedia.profileURL)}</h3>
                {/* Edit socialMedia */}
                <form className="form">
                    <input type="text" name="socialMedia" id="socialMedia" className='input'/>
                    <button type="submit">Submit</button>
                </form>
                <br/>
                ...
                <br/>
                <h3>Dislikes: {useCurrentUser().dislikes.map((dislike) => dislike.name)}</h3>
                {/* Edit dislikes */}
                <form className="form">
                ...
                <br/>
                <h3>Favorited Users: {useCurrentUser().favoritedUsers.map((favoritedUser) => favoritedUser.id)}</h3>
                {/* Edit favoritedUsers */}
                <form className="form">
                    <input type="text" name="favoritedUsers" id="favoritedUsers" className='input'/>
                    <button type="submit">Submit</button>
                </form>
                <br/>
            </div>
